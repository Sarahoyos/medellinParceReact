...
const ProductosSweaters = () => {
  ...
  return (
    <div>
      <header>
        <div className="logo"><a href="/home"><img src="/logo.png" alt="logo" /></a></div>
        <nav>
          <ul>
            ...
            {!usuario && <li id="auth-section"><Link to="/login">INICIAR SESION</Link></li>}
            {usuario && (
      ...
      <main className="main-content">
        <h1 className="category-title">Sweaters</h1>
        {cargando && <p style={{ textAlign: "center", padding: "40px" }}>Cargando productos...</p>}
        {error && <p style={{ textAlign: "center", color: "#cc0000", padding: "20px" }}>{error}</p>}
        {!cargando && !error && productos.length === 0 && (
          <p style={{ textAlign: "center", padding: "40px" }}>No hay productos disponibles.</p>
        )}
        <div className="product-grid">
          {productos.map((p) => (
            <div className="product-card" key={p.idProducto}>
              ...
              <p style={{ fontSize: "14px", color: "#555", margin: "4px 0" }}>Talla: {p.talla} | Color: {p.color}</p>
              <p className="product-price">{formatearPrecio(p.precio)}</p>
              <button className="add-to-cart-btn" onClick={() => addToCart(p.nombreProducto, p.precio, p.idProducto, usuario)}>
                Agregar
              </button>
            </div>
          ))}
...
export default ProductosSweaters;
